...
import { LayoutDashboard, ShieldCheck, Building, FileText,
  ScrollText, FileCheck, BookOpenText, CheckSquare, ClipboardList, Users, Zap, FileDown, X, ShieldAlert, Wrench, FileCog, MapPinned, ClipboardCheck, FileBarChart2, BellRing, ScanSearch, CalendarSync } from 'lucide-react';
...
export default function Sidebar({ isMobileMenuOpen, setIsMobileMenuOpen }: SidebarProps) {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-[86vw] max-w-72 bg-surface border-r border-border flex h-screen flex-col transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 md:sticky md:top-0 md:translate-x-0 md:w-72 overflow-hidden md:min-w-[240px]`}>
      <div className="p-4 md:p-8 flex items-center justify-between gap-3 border-b border-border relative overflow-hidden">
        ...
        <div className="flex items-center gap-2 md:gap-3 relative z-10 min-w-0">
          <img src="/acreplast-logo.png" alt="Acreplast" className="w-32 md:w-40 h-12 md:h-16 object-contain object-left shrink-0" id="sidebar-logo" />
        </div>
        <button 
      ...
      <nav className="flex-1 px-4 py-6 md:py-8 space-y-2 overflow-y-auto overflow-x-hidden overscroll-contain w-full sidebar-scrollbar">
        ...
        <SideNavItem icon={<CalendarSync size={18} />} label="Transição NR-10 2026" to="/nr10-2026" id="nav-nr10-2026" onNavigate={closeMobileMenu} />
